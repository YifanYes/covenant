...
export default function CharacterStatus({ character }: CharacterStatusProps) {
  ...
  const currentClass =
    character && character.classes.find((characterClass) => characterClass.className === character.currentClass)
  const isDead = (currentClass?.health ?? 0) <= 0

  if (!currentClass) return null

  return (
    <Card className='relative flex min-h-0 w-full flex-1 flex-col gap-0 overflow-hidden'>
      <div className={cn('flex flex-1 flex-col transition-all duration-300', isDead && 'opacity-50 grayscale')}>
