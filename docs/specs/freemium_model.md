...
Covenant needs sustainable revenue without compromising its core philosophy: **mana is earned, never bought**.

- Patreon integration adds third-party dependency and operational overhead.
- Microtransactions per-item conflict with the "no shortcuts" brand and require ongoing content pipelines.

A freemium subscription model is the standard SaaS pattern that works at pre-scale: free tier hooks users into the full RPG loop, premium unlocks depth and convenience without pay-to-win implications.

The model must never gate mana, combat access, or basic productivity functions — those are the core loop and must remain free to preserve trust.
