...
8. **Post-spawn turn-pointer recovery.** Kill enemy 1, spawn enemy 2, attack enemy 2 once without waiting for the enemy-turn animation to settle. Then immediately attempt a second player move. Pass: second move resolves (auto-runs the pending enemy turn first, then the player move), never throws `Not this unit's turn`. Repro for the bug fixed here: BE state with `turnOrder=[player, enemy]`, `currentTurnIndex=1` previously left the player stuck.
9. **Potion usable across enemy turns.** Mid-encounter: use a health potion (the FE should grey it out for the rest of the turn). Wait for the enemy turn to land. Pass: potion button is interactable again on the next player turn, and refreshing the page does not re-block it (BE state's `potionUsedThisTurn` cleared after enemy turn, not just FE local state).
10. **DOT-kill spawns next enemy.** Apply BURNING / PURIFIED to a non-demon enemy, or POISONED to any enemy, with damage that will leave the enemy at ≤ DOT tick HP. End the player turn. Pass: enemy dies at start of its turn from the DOT tick, gold is credited, next enemy spawns, kill counts toward quest progress. Pre-fix behavior: enemy showed dead but nothing spawned; quest progress stalled.
