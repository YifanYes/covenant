...
Internet
  ...
  -> Hetzner CAX21 VPS (FSN1, Falkenstein)
...
Estimated total: **~€10/mo**, vs ~$20–40 on Railway. Savings are real (≈50–70%) but the **primary motivation is hands-on DevOps learning** — Docker, Traefik, Let's Encrypt, Postgres ops, SSH hardening, Cloudflare proxy, GitHub webhooks. Cost savings are secondary.
...
**Hetzner CAX21 (Ampere Arm64, 4 vCPU / 8 GB / 80 GB NVMe / 20 TB egress)** in **FSN1 Falkenstein** at **~€7.59/mo** post-April-2026 price increase. Closest Hetzner EU region to Madrid (~25 ms RTT). Arm is chosen because Coolify, Next.js, Postgres, and Redis all run cleanly on Arm64 and Arm is consistently the cheapest tier. Upgrade path is CPX32 (4 vCPU AMD / 8 GB / 160 GB, ~€15/mo) or move builds off-box if local Next.js builds OOM.
...
| --- | --- | --- | --- |
| **CAX21 (Arm, FSN1)** | 4 / 8 GB / 80 GB | ~€7.59/mo | **Recommended.** Headroom for app + DB + Redis + builds. |
| CAX11 (Arm, FSN1) | 2 / 4 GB / 40 GB | ~€3.79/mo | Too tight once Coolify (~1 GB idle) + Postgres + builds run. |
| CPX22 (x86 AMD) | 3 / 4 GB / 80 GB | ~€7.99/mo | Same price, less RAM, x86 only. Pick only if an Arm image is unavailable for some dependency. |
| CPX32 (x86 AMD) | 4 / 8 GB / 160 GB | ~€15/mo | Upgrade target if builds OOM or disk fills. |
...
| Item | Monthly | Notes |
| --- | --- | --- |
| Hetzner CAX21 (Arm, FSN1) | €7.59 | Post-April-2026 pricing. |
| Hetzner automated backups (+20%) | €1.52 | Daily snapshots, 7-day retention. |
...
| **Total** | **~€10/mo** | vs Railway ~$20–40/mo → ~50–70% savings. |
...
1. Create Hetzner project; deploy **CAX21** in **FSN1**, Ubuntu 24.04 LTS, with SSH key pre-installed.
...
7. Add 2 GB swap (`fallocate /swapfile`, `mkswap`, `swapon`, `/etc/fstab` entry).
...
1. Schedule a known idle window (no real users, so anytime).
2. From Railway: `pg_dump` the production DB to a local file. Encrypt at rest if storing temporarily.
3. On Hetzner: `pg_restore` into the Coolify-managed Postgres. Run `pnpm prisma db push` if schema needs syncing after restore.
4. Smoke-test the staging URL against the restored data: read main entities (characters, guilds, journal entries), perform a write, confirm Redis-backed rate limits work.

**Verify**: row counts match between source and target for each table; an end-to-end user flow works.
...
- **RPO**: 24 hours. **RTO**: ~1–2 hours to rebuild a fresh CAX21 from snapshot + Coolify reinstall + DB restore + Cloudflare A-record flip. Document the exact steps in `docs/runbooks/disaster_recovery.md` (separate PR).
...
| **RAM / disk contention during Next.js builds** on a 4-vCPU / 8-GB box. | 2 GB swap pre-allocated. If builds OOM, move builds to GitHub Actions and push the image to Coolify's registry; the VPS only runs containers. |
...
| **ICANN 60-day transfer lock.** | If the domain was registered or contact-changed in the last 60 days, wait out the lock before Phase 5. |
