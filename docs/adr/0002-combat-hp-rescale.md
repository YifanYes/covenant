...
- **Reduce `power` values instead** — keep HP, push `basic_strike` from 35 to ~5 and doctrines down accordingly. Rejected: loses the Pokémon "feel" of power numbers (Tackle 35, Hyper Beam 150) that the rewrite is borrowing for player intuition.
...
- One-time edit across `classes.ts`, `enemies.ts`, `enemy-ai.ts` (DOT ticks), `doctrines.ts` (thorns), `items.ts` (health_potion). All values readable as final literals — no factor lookup.
