...
export class CharacterRepository {
  ...
  // Material methods
  async getMaterials(characterId: string): Promise<Record<string, number>> {
    const character = await this.prisma.character.findUnique({
      where: { id: characterId },
      select: { materials: true }
    })
    return (character?.materials as Record<string, number>) || {}
  }

  async updateMaterials(characterId: string, materials: Record<string, number>): Promise<void> {
    await this.prisma.character.update({
      where: { id: characterId },
      data: { materials }
    })
  ...
  async addMaterials(characterId: string, materialsToAdd: Record<string, number>): Promise<Record<string, number>> {
    const currentMaterials = await this.getMaterials(characterId)

    const newMaterials = { ...currentMaterials }
    for (const [materialId, quantity] of Object.entries(materialsToAdd)) {
      newMaterials[materialId] = (newMaterials[materialId] || 0) + quantity
    }

    await this.updateMaterials(characterId, newMaterials)
    return newMaterials
  }

  async removeMaterials(characterId: string, materialsToRemove: Record<string, number>): Promise<Record<string, number>> {
    const currentMaterials = await this.getMaterials(characterId)

    const newMaterials = { ...currentMaterials }
    for (const [materialId, quantity] of Object.entries(materialsToRemove)) {
      const current = newMaterials[materialId] || 0
      newMaterials[materialId] = Math.max(0, current - quantity)
  ...
  async updateMaterialsInventoryAndGold(
    characterId: string,
    ...
    return this.prisma.character.update({
      where: { id: characterId },
      data: { materials, inventory: inventory as any, gold }
    })
  }
