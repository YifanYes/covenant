...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    h1: ({ children }) => (
      <h1 className="font-title text-primary mb-6 text-4xl font-bold">{children}</h1>
    ),
    h2: ({ children }) => (
      <h2 className="font-title text-primary mb-4 mt-8 text-2xl font-bold">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="font-title text-primary mb-3 mt-6 text-xl font-semibold">{children}</h3>
    ),
    p: ({ children }) => (
