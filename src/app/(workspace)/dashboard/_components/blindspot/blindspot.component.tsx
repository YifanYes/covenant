'use client'
import { Notes, SquareAlert, Target, Trophy } from 'pixelarticons/react'
...
interface BlindspotComponentProps {
  ...
  habits: { name: string; lastCompletion: string | null }[]
}

export default function BlindspotComponent({ areas, objectives, habits = [] }: BlindspotComponentProps) {
  ...
  const filteredHabits = useMemo(() => getBlindspotListData(habits), [habits])
  const hasAnyBlindspots = filteredAreas.length > 0 || filteredObjectives.length > 0 || filteredHabits.length > 0
  ...
  return (
    <DashboardSectionWrapperComponent
      ...
      contentClassName={
        hasAnyBlindspots ? 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3' : 'flex items-center justify-center'
      }
    >
      {hasAnyBlindspots ? (
        <>
          ...
          <div className="border-border bg-card/20 flex max-h-50 flex-col rounded-lg border px-4 py-3">
            <div className="text-foreground mb-2 flex shrink-0 items-center gap-2 text-sm font-bold">
              ...
              {t('dashboard.blindspot.habits')}
            </div>
            <div className="scrollbar-thin scrollbar-thumb-muted h-full overflow-y-auto">
              <BlindspotListComponent items={filteredHabits} emptyKey="dashboard.blindspot.all_habits_covered" />
            </div>
          </div>
        </>
      ) : (
