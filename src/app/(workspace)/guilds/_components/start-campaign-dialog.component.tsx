...
import { CAMPAIGN_TEMPLATES } from '@shared/constants/guild-campaigns'
...
export default function StartCampaignDialog({ guildId, trigger }: StartCampaignDialogProps) {
  ...
  const [selectedId, setSelectedId] = useState<string>(CAMPAIGN_TEMPLATES[0]?.id ?? '')
  ...
  return (
    <BaseFormDialog
      ...
      onSubmit={() => mutation.mutate({ guildId, templateId: selectedId })}
      submitLabel="guilds.campaigns.start.submit"
      isLoading={mutation.isPending}
      isSubmitDisabled={!selectedId}
